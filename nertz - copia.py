...
from models import (
    Base, MarketData, Orderbook, MarketTicker, Trade, Position, init_db
)
...
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://postgres:angel_amely@localhost:5432/trading_db")
...
class NertzMetalEngine:
    ...
    async def _handle_kline(self, symbol: str, kline: Dict, db: Session) -> None:
        ...
        existing_candle = db.query(MarketData).filter_by(timestamp=candle.timestamp, symbol=symbol).first()
    ...
    async def _handle_ticker(self, symbol: str, ticker: Dict, db: Session) -> None:
        try:
            ...
            # Crear un diccionario con los valores actualizados para evitar problemas con **
            ticker_data = {
                "timestamp": datetime.now(timezone.utc),
                "symbol": symbol,
    ...
    async def _execute_trade(self, symbol: str, db: Session) -> None:
        ...
        if order_result["success"]:
            trade = Trade(
                ...
                volatility=float(metrics["volatility"]) if hasattr(metrics['volatility'], 'item') else float(metrics['volatility']),
